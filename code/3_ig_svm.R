...
tr <- rbind(snp.fold3,snp.fold4,snp.fold5)
val <- snp.fold2
...
importance <- read.csv(file = 'D:/workspace_R/thalas/20180427/result/mutual/mutual_val2_test1.csv',header = T)
colnames(importance) <- 'ig'
importance$nm <- colnames(tr.x)


importance <- importance[order(-importance$ig),] 
plot(importance$ig)
importance2 <- importance[1:500,]
...
for (i in 2:200){
  ...
  for (k.for in 1:length(param.c)){
    ...
    acc <- confusionMatrix(reference = te.y$te.case,result)
...
write.csv(x = tmp.write, file = 'D:/workspace_R/thalas/20180427/result/chi/svm/result_chi_svm_val1_test5.csv',row.names = F)
